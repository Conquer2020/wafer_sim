...
class Tile():# for compute process
    ...
    def analysis_forward_process(self,env,map_ana:list,device:List[int],op_list:List[OpNode],wd1:wd):
        def analysis_template_event(param=[None,None,None,None,None,None,None,None,None,None,None]):
            ...
            if(param[0]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[0]*self.buffer_bytes*len(device),group_id=device,task_id=event.wt_load,multicast=False))
            if(param[1]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[1]*self.act_bytes*len(device),group_id=device,task_id=event.act_fetch,multicast=False))
            if(param[2]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[2]*self.buffer_bytes,device,event.wt_load,WRITE=False))
            if(param[3]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[3]*self.act_bytes,device,event.act_fetch,WRITE=False))
            if(param[4]!=None):
                #ZeRO communication 
                event_list.append(self.tile_comm_process(param[4],wd1,event.comm))
            if(param[5]!=None):
                #compute 
                event_list.append(self.tile_comp_process(param[5]))
            if(param[6]!=None):
                #communication 
                event_list.append(self.tile_comm_process(param[6],wd1,event.comm))
            if(param[7]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[7]*self.act_bytes,device,event.act_store,WRITE=True))
            if(param[8]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[8]*self.act_bytes,device,event.act_store,WRITE=True))
            if(param[9]!=None):
                event_list.append(wd1.dram_write_group_process(access_size_MB=param[9]*self.act_bytes*len(device),group_id=device,task_id=event.act_store,gather=True))
            if(param[10]!=None):
                event_list.append(wd1.dram_write_group_process(access_size_MB=param[10]*self.act_bytes*len(device),group_id=device,task_id=event.act_store,gather=True))
            return event_list
    ...
    def analysis_backward_process(self,env,map_ana:list,device:List[int],op_list:List[OpNode],wd1:wd):
        ...
        def analysis_template_dloss_event(param=[None,None,None,None,None,None,None,None,None]):  
            ...
            if(param[0]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[0]*self.buffer_bytes*len(device),group_id=device,task_id=event.wt_load,multicast=False))
            if(param[1]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[1]*self.act_bytes*len(device),group_id=device,task_id=event.grad_fetch,multicast=False))
            if(param[2]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[2]*self.buffer_bytes,device,event.wt_load,WRITE=False))
            if(param[3]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[3]*self.act_bytes,device,event.grad_fetch,WRITE=False))
        ...
        def analysis_template_dW_event(param=[None,None,None,None,None,None,None,None,None,None,None,None,None]):  
            ...
            event_list=[]
            if(param[0]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[0]*self.act_bytes*len(device),group_id=device,task_id=event.act_fetch,multicast=False))
            if(param[1]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[1]*self.act_bytes*len(device),group_id=device,task_id=event.grad_fetch,multicast=False))
            if(param[2]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[2]*self.full_bytes*len(device),group_id=device,task_id=event.opt_load,multicast=False))
            if(param[3]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[3]*self.act_bytes,device,event.act_fetch,WRITE=False))
            if(param[4]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[4]*self.act_bytes,device,event.grad_fetch,WRITE=False))
            if(param[5]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[5]*self.full_bytes,device,event.opt_load,WRITE=False))
            if(param[6]!=None):
                #ZeRO communication 
                event_list.append(self.tile_comm_process(param[6],wd1,event.comm))
            if(param[7]!=None):
            ...
            if(param[8]!=None):
                #communication 
                event_list.append(self.tile_comm_process(param[8],wd1,event.comm))
            if(param[9]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[9]*self.full_bytes,device,event.opt_store,WRITE=True))
            if(param[10]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[10]*self.full_bytes,device,event.wt_store,WRITE=True))
            if(param[11]!=None):
    ...
    def analysis_weight_update_process(self,env,map_ana,device:List[int],op_list:List[OpNode],wd1:wd):
        def analysis_template_event(param=[None,None,None,None,None,None,None]):
            ...
            event_list=[]
            if(param[0]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[0]*self.full_bytes*len(device),group_id=device,task_id=event.wt_load,multicast=False))
            if(param[1]!=None):
                event_list.append(wd1.dram_read_group_process(access_size_MB=param[1]*self.full_bytes*len(device),group_id=device,task_id=event.grad_fetch,multicast=False))
            if(param[2]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[2]*self.full_bytes,device,event.wt_load,WRITE=False))
            if(param[3]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[3]*self.full_bytes,device,event.grad_fetch,WRITE=False))
            if(param[4]!=None):
                #DP communication 
                event_list.append(self.tile_comm_process(param[4],wd1,event.comm,True))
            if(param[5]!=None):
                event_list.append(wd1.tile_dram_group_access_process(param[5]*self.full_bytes,device,event.wt_load,WRITE=True))
            if(param[6]!=None):
                event_list.append(wd1.dram_write_group_process(access_size_MB=param[6]*self.full_bytes*len(device),group_id=device,task_id=event.wt_load,gather=True))
            return event_list
        dataflow0,sram1,recomputes2,tiledram3,edgedram4=map_ana
