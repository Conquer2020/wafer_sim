...
class Wafer_Device:
    ...
    def edge_dram_write_process(
        ...
        while True:
            # if DEBUG_MODE:
            last_time = self.env.now
            print("task {} start dram wrtie  @ {:.3f} ms".format(task_id, self.env.now))
            if des_id != src_id:
                yield self.env.process(
                    self.noc_process(
                        access_size_MB,
            ...
            if not self.Analytical:
                ...
                # print(dram_index)
                # print('self.edge_dram_resource[dram_index].bw_GB',self.edge_dram_resource[dram_index].bw_GB)
                yield self.env.process(
                ...
                print("edge_die_dram_bw_GB", self.edge_die_dram_bw_GB)
                yield self.env.timeout(
                    self.dram_response_latency_ms
            ...
            # if DEBUG_MODE:
            print(
                "task {} dram write lasting @ {:.3f} ms".format(
                    task_id, self.env.now - last_time
                )
            )
            break
    ...
    def dram_read_group_process(
        ...
        while True:
            ...
            print(
                "task {} dram_read_group_process @ {:.3f} ms".format(
                    task_id, self.env.now - last_time
                )
            )
            break
    ...
    def dram_write_group_process(
        ...
        while True:
            ...
            print(
                "task {} dram_write_group_process @ {:.3f} ms".format(
                    task_id, self.env.now - last_time
                )
            )
            break
...
def validate_congestion1():
    ...
    for comm_size in comm_sizes:
        ...
        env.process(
            wd.dram_read_group_process(
                access_size_MB=comm_size,
                group_id=[0, 1, 2, 3, 4, 5, 6, 7, 15, 14, 13, 12, 11, 10, 9, 8],
                task_id="ALL_REDUCE_process",
            )
        )
...
def validate_congestion1000():
    # NOTE: below is 16 NPUs
    # comm_sizes = [64, 96, 128, 192, 768, 1536]
    print(f"$$$$$$$$$$$$$$$$Analytical Model$$$$$$$$$$$$$$$$")
    comm_sizes = [0.1, 0.5, 1, 2, 4, 8, 10, 128]
    for comm_size in comm_sizes:
        for analytical in [True, False]:
            print(f"&&&&&&&&&&&&&&--{comm_size} MB--&&&&&&&&&&&&&&&&&&")
            Debug = True
            env = simpy.Environment()
            wd = Wafer_Device(
                env,
...
def validate_congestion2000():
    # NOTE: below is 16 NPUs
    # comm_sizes = [64, 96, 128, 192, 768, 1536]
    print(f"$$$$$$$$$$$$$$$$Analytical Model$$$$$$$$$$$$$$$$")
    comm_sizes = [0.1, 0.5, 1, 2, 4, 8, 10]
    for comm_size in comm_sizes:
        for analytical in [True, False]:
            print(f"&&&&&&&&&&&&&&--{comm_size} MB--&&&&&&&&&&&&&&&&&&")
            Debug = True
...
def validate_congestion5000():
    # NOTE: below is 16 NPUs
    # comm_sizes = [64, 96, 128, 192, 768, 1536]
    print(f"$$$$$$$$$$$$$$$$Analytical Model$$$$$$$$$$$$$$$$")
    comm_sizes = [0.1, 0.5, 1, 2, 4, 8, 10, 1024]
    for comm_size in comm_sizes:
        for analytical in [True, False]:
            print(f"&&&&&&&&&&&&&&--{comm_size} MB--&&&&&&&&&&&&&&&&&&")
            Debug = True
...
if __name__ == "__main__":
    # validate_congestion0()
    # validate_congestion1()
    ...
    # validate_congestion10()
    # validate_congestion100()
    # validate_congestion00()
    # validate_congestion11()
    # validate_congestion22()

    # validate_congestion0000()
    # validate_congestion1000()
    # validate_congestion2000()
    # validate_congestion3000()
    # validate_congestion4000()
    validate_congestion5000()
