...
class OpNode(Oppd):
    ...
    def __str__(self):
        if self.dpmap_flag:
            return (
                "{}:({},{}),p_sgy={},device={},parent_nodes:{},child_nodes:{}\n".format(
                    self.hint_name,
                    self.type,
                    self.param_dim,
                    self.p_sgy,
...
class CompGraph:
    def __init__(
        self, root: str = None, name="t_Compute_Graph", meta="20230424"
    ) -> None:
        self.name = name
        self.meta = meta
        self.root = root
    ...
    def gread(path="test", name="gh"):
        ...
        for items in gpdict:
            if i == 0:
                ...
                op_plm_dim = None
                op_device = None
                # NOTE: op_key in (type, param_dim, child_nodes, parallel_strategy, device)
                ...
                ops_dict[op_hint_name] = OpNode(
                    op_type=op_type, op_param=op_param, hint_name=op_hint_name
                )
